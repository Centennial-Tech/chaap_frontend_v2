...
const mockChecklistItems: ChecklistItem[] = [
  {
    ...
    description: "Comprehensive description of the device including principles of operation",
    status: "completed",
...
export function SubmissionChecklist({ submissionId }: SubmissionChecklistProps) {
      ...
      <CardContent>
        {checklistItems && checklistItems.length > 0 ? (
          <>
            ...
            <div className="space-y-4">
              {checklistItems.map((item) => (
                <div
                  key={item.id}
                  className={`flex items-start space-x-3 p-4 rounded-lg border ${getCardBgColor(item.status)}`}
                >
            ...
            <Alert className="mt-6 bg-blue-50 border-blue-200">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="font-medium text-blue-900 mb-1">Progress Summary</h4>
                  <AlertDescription className="text-blue-700">
                    {checklistItems.filter(item => item.status === "completed").length} of {checklistItems.length} sections completed. 
                    {checklistItems.filter(item => item.status === "review_needed").length > 0 && (
                      <> {checklistItems.filter(item => item.status === "review_needed").length} section{checklistItems.filter(item => item.status === "review_needed").length > 1 ? 's' : ''} need{checklistItems.filter(item => item.status === "review_needed").length === 1 ? 's' : ''} review.</>
                    )}
                    {progress === 100 ? " Your submission is ready for final review!" : " Continue working on pending sections."}
                  </AlertDescription>
                </div>
              </div>
            </Alert>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">
              Checklist will be generated based on your submission pathway and requirements.
            </p>
          </div>
        )}
      </CardContent>
