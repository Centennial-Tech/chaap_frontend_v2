...
import { CalendarDays, Edit, Plus, CheckCircle, Clock, Circle } from "lucide-react";
        ...
        {timelineItems.length > 0 ? (
          <>
              ...
              <div className="space-y-6">
                {timelineItems.map((item) => (
                  <div key={item.id} className="relative flex items-start space-x-4">
                    <div className={`flex items-center justify-center w-8 h-8 rounded-full z-10 ${getStatusColor(item.status)}`}>
                      {getStatusIcon(item.status)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-medium text-gray-900">{item.phase}</h4>
                        <span className="text-sm text-gray-500">{item.weekRange}</span>
                      </div>
                      <p className="text-sm text-gray-600 mb-3">{item.description}</p>
                      <div className="flex items-center space-x-4 text-sm">
                        <Badge className={item.status === "completed" ? "bg-teal-100 text-teal-800" : 
                                       item.status === "in_progress" ? "bg-yellow-100 text-yellow-800" : 
                                       "bg-gray-100 text-gray-800"}>
                          {getStatusLabel(item.status)}
                        </Badge>
                        <span className="text-gray-500">Target: {item.targetDate}</span>
                        <select
                          value={item.status}
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <div className="text-sm text-gray-500">
                Total Duration: {totalDuration} weeks | Critical Path: Testing → Documentation → Submission
              </div>
            </div>
          </>
        ) : (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 mb-4">
              Timeline will be generated based on your submission pathway and testing requirements.
            </p>
            <Button>
